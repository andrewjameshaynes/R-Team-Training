...
In this section we will briefly discuss data operations such as reading and writing data, merging data, filtering data and aggregating data. We'll show how to do this in base R and using the `tidyr` and `dplyr` packages.
...
R comes with a lot of pre-buit modelling functions all written by statistics professors, and often optimised in base C for efficiency. They also come with a plethora of options on which implementation of a model you would like. As such this along with R's easy to sue data structures has made it a popular choice for Data Science and Analytics.



# simple lm toexmplain the ~ operator and the summary functions

# ya boy glm 

# throw ya a logistic regression and  the predict function
